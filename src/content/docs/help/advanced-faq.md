...
They are different local measurements. Overall progress weights each coin's score toward the target by its value, while the colored private balance counts the value already meeting that target. Neither is a measured probability that an outside observer can identify you; the cost-and-progress guide contains the worked calculation.
